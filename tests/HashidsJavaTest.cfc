component extends="mxunit.framework.TestCase" output="false" persistent="false" {
	...
	*/
	public void function test_one_number(){
		...
		var results = hasher.encrypt(input);

		// make sure the string encrypts as expected
		assertEquals(expected, results);
		// make sure the string decrypts back to the original value
		assertEquals(input, hasher.decrypt(results));
	}
	...
	public void function test_randomness_for_incrementing(){
		...
		assertEquals(hasher.encrypt(1), "NV");
		assertEquals(hasher.encrypt(2), "6m");
		assertEquals(hasher.encrypt(3), "yD");
		assertEquals(hasher.encrypt(4), "2l");
		assertEquals(hasher.encrypt(5), "rD");
	}
	...
	public void function test_for_values_greater_int_maxval(){
		...
		// test the input for randomness
		assertEquals(hasher.encrypt(9876543210123), "Y8r7W1kNN");
	}
