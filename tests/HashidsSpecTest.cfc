component extends="mxunit.framework.TestCase" output="false" persistent="false" {
	...
	*/
	...
	public void function should_pass_basic_encoding(){
		var hashids = new com.utils.Hashids();
		var numbers = [1, 2, 3, 4, 5];

		var hash = hashids.encode(numbers);
		var returnedNumbers = hashids.decode(hash);

		// expect the hash to have some length
		assertTrue(len(hash));
		// expect the input to match the decoded value
		assertEquals(numbers, returnedNumbers);
	}

	public void function should_pass_encoding_by_passing_a_hex_string_mongodb_test(){
		var hashids = new com.utils.Hashids();
		var hex = "507f1f77bcf86cd799439011";

		var hash = hashids.encodeHex(hex);
		var returnedHex = hashids.decodeHex(hash);
	...
	public void function should_pass_encoding_by_passing_a_hex_string_mongodb_test_with_minimum_hash_length_parameter(){
	...
	public void function should_pass_encoding_by_passing_a_long_hex_string_mongodb_test(){
		var hashids = new com.utils.Hashids();
	...
	public void function should_pass_basic_encoding_with_salt_value(){
		...
		assertNotEquals(hash, new com.utils.Hashids().encode(numbers));
	}

	public void function should_pass_encoding_with_salt_value_and_minimum_hash_length(){
		var minHashLength = 1000;
	...
	public void function should_pass_encoding_with_salt_value_and_big_minimum_hash_length(){
	...
	public void function should_pass_encoding_with_salt_value_minimum_hash_length_and_custom_alphabet(){
		var minHashLength = 1000;
